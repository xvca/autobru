...
```sh
cp .env.example .env
# Edit .env: set ESP_IP and PIO_ENV to match your device.
./scripts/upload-firmware.sh --dry-run
./scripts/upload-firmware.sh --build
```

Without `--build`, the script uploads the existing `.pio/build/$PIO_ENV/firmware.bin`; it does not rebuild it. Exported environment variables override `.env`. The script asks for confirmation before flashing and rebooting—only update while the machine is idle. Use `--yes` to skip that prompt.
